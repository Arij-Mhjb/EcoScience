...
const ITEMS = [
  { id: "newspaper", label: "جريدة", emoji: "📰" },
  { id: "cardboard", label: "كرتون", emoji: "📦" },
  { id: "glass_jar", label: "برطمان زجاجي", emoji: "🫙" },
  { id: "metal_can", label: "علبة معدنية", emoji: "🥫" },
  { id: "plastic_bottle", label: "زجاجة بلاستيك", emoji: "🧴" },
  { id: "shirt", label: "قميص سليم", emoji: "👕" },
  { id: "toy", label: "لعبة سليمة", emoji: "🧸" },
  { id: "battery", label: "بطارية", emoji: "🔋" },
] as const;
...
const CORRECT_ANSWERS: Record<ItemId, ZoneId> = {
  ...
  cardboard: "paper",
  glass_jar: "glass",
  metal_can: "plastic_metal",
  plastic_bottle: "plastic_metal",
  shirt: "donate",
  toy: "donate",
  battery: "special",
};
...
const STATE_CLASSES: Record<ItemState, string> = {
  ...
  selected:
    "border-2 border-primary bg-primary-50 ring-2 ring-primary ring-offset-2 scale-105 cursor-pointer",
...
export default function DragDropChallenge({
  ...
  const getItemState = (itemId: ItemId): ItemState => {
    if (verified)
      return placements[itemId] === CORRECT_ANSWERS[itemId]
        ? "correct"
        : "incorrect";
  ...
  const getZoneItems = (zoneId: ZoneId) =>
    ITEMS.filter((item) => placements[item.id] === zoneId);
  ...
  const turtleMessage = verified
    ? score >= 16
      ...
      : `أحسنت! حصلت على ${score} نقطة! تدرّب أكثر 💪`
    : selectedItem
      ? `اختر مكان ${selectedItemData?.emoji} ${selectedItemData?.label}! ⬇️`
      : "اختر شيئاً ثم ضعه في مكانه الصحيح! 🎯";
    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-kid shadow-kid p-4 mb-5">
        <p className="text-xs font-semibold text-gray-400 text-center mb-3 tracking-wide uppercase">
          الأشياء — انقر لاختيار
        </p>
        <div className="grid grid-cols-4 gap-2 sm:gap-3">
          {ITEMS.map((item) => {
            const state = getItemState(item.id);
            const placedZone = placements[item.id]
              ? ZONES.find((z) => z.id === placements[item.id])
              : null;
            ...
            return (
              <motion.button
                ...
                whileHover={
                  !verified && state !== "selected" ? { scale: 1.06 } : {}
                }
                whileTap={!verified ? { scale: 0.94 } : {}}
                className={`relative p-2 sm:p-3 rounded-kid transition-all duration-200
              ...
              >
                ...
                {/* Zone actuellement assignée (état placé) */}
                {state === "placed" && placedZone && (
                  <span className="text-[9px] text-gray-400 font-medium text-center leading-tight">
                    {placedZone.emoji} {placedZone.label}
                  </span>
                )}

                {/* Badge résultat après vérification */}
                {verified && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
        ...
        {ZONES.map((zone) => {
          ...
          return (
            <motion.div
              ...
              whileHover={isDropTarget ? { scale: 1.04, y: -2 } : {}}
              whileTap={isDropTarget ? { scale: 0.97 } : {}}
              className={`flex-1 min-w-[130px] border-2 rounded-kid p-3 min-h-[100px]
                          transition-all duration-200 ${zone.color}
                          ${
                            isDropTarget
                              ? "cursor-pointer shadow-kid ring-2 ring-primary/30"
                              : "cursor-default"
                          }`}
            >
          ...
          >
            ...
            >
              ...
              <p className="font-black text-xl text-gray-800 mb-1">
                {score >= 16 ? "ممتاز! أنت بطل التدوير!" : "جيد! تعلّمنا معاً!"}
              </p>
              <p className="text-gray-500 text-sm">
                {ITEMS.length - errors} إجابات صحيحة من {ITEMS.length}
              </p>
            </div>
            ...
            {errors > 0 && (
              ...
              >
                ...
                <div className="space-y-2">
                  {ITEMS.filter(
                    (item) => placements[item.id] !== CORRECT_ANSWERS[item.id],
                  ).map((item) => {
                    const correctZone = ZONES.find(
                      (z) => z.id === CORRECT_ANSWERS[item.id],
                    );
                    return (
                      <div
                        key={item.id}
                        className="flex items-center gap-2 text-sm text-red-600 text-right justify-end"
                      >
                        <span className="font-semibold">
                          {correctZone?.label}
                        </span>
                        <span className="text-base">{correctZone?.emoji}</span>
                        <span className="text-gray-400 text-xs">ينتمي إلى</span>
