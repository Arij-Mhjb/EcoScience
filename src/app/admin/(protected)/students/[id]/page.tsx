...
export default function StudentDetailPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([
      fetch(`/api/admin/students/${id}`).then(async r => { try { return await r.json() } catch { return { user: null } } }),
      fetch('/api/admin/contests').then(async r => { try { return await r.json() } catch { return { contests: [] } } }),
    ]).then(([sd, cd]) => {
      if (!sd?.user) { setLoading(false); return; }
      setStudent(sd.user);
