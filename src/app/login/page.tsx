// PAGE 2 — Sign In / Connexion
// Formulaire de connexion avec la tortue mascotte
'use client';
...
export default function LoginPage() {
  ...
  const [turtleMsg, setTurtleMsg] = useState('مرحبا! أدخل بياناتك للبدء 🐢');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        setTurtleMood('sad');
        setTurtleMsg('يبدو أن هناك خطأ! حاول مرة أخرى 😕');
      } else {
        setTurtleMood('happy');
        setTurtleMsg('مرحبا بك! هيا بنا! 🎉');
        setTimeout(() => router.push('/dashboard'), 1000);
      }
    } catch {
      setError('حدث خطأ غير متوقع');
      setTurtleMood('sad');
      setTurtleMsg('عذرا! حدث خطأ 😢');
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen bg-ocean-gradient flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-ocean-light/80 mt-1">تسجيل الدخول</p>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-kid shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                onFocus={() => { setTurtleMood('idle'); setTurtleMsg('أدخل بريدك الإلكتروني 📧'); }}
                placeholder="student@ecoscience.ma"
                className="w-full p-4 rounded-kid border-2 border-gray-200 focus:border-primary
                           focus:ring-2 focus:ring-primary-200 outline-none transition-all text-right
            ...
            <div>
              ...
              <input
                ...
                onFocus={() => { setTurtleMood('idle'); setTurtleMsg('أدخل كلمة المرور 🔐'); }}
                placeholder="••••••••"
                className="w-full p-4 rounded-kid border-2 border-gray-200 focus:border-primary
                           focus:ring-2 focus:ring-primary-200 outline-none transition-all text-right
                           text-lg placeholder:text-gray-300"
          ...
          <p className="text-center text-gray-400 text-xs mt-6">
            الحسابات يتم إنشاؤها من طرف الفريق التعليمي
          </p>
        </div>
      </motion.div>
    </main>
